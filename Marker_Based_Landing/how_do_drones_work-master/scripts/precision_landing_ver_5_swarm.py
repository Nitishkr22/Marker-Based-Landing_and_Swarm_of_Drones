...
class Swarm_and_land():
    ...
    def initiate_func(self,vehicle_m):
        ...
        self.sleep(2)
        #logging.info('Setting yaw to zero')
        ...
        while True:
            ...
            time_0 = time.time()
             logging.info("Altitude greater than ")
            ...
            logging.info(range_sensor_alt)
            
            logging.info(vehicle.location.local_frame)
            if self.land_flag == 1:
            ...
            if marker_found:
               ...
               if yaw_flag == 0:
                ...
                cur_altitude = z_m
                -- If high altitude,if self.land_flag == 0: use baro rather than visual
                ...
                if time.time() >= time_0 + 1.0/freq_send:
                   ...
                   logging.info(" ")
                    ...
                    logging.info("Altitude range sensor = %.0fm" % range_sensor_alt_m)
                   logging.info("Marker found x = "+str(x_cm)+"  y = "+str(y_cm))
                    
                    logging.info("Marker N = -"+str(x_cm)+"  E = -"+str(y_cm))
                    
                    logging.info('moving drone to, N=', vehicle.location.local_frame.north -
                    ...
                    self.goto_ned(x_cm,y_cm)
                    alt = -vehicle.location.local_frame.down #change here
                    alt = vehicle.location.global_relative_frame.alt #change here
                    logging.info("local-alt: -"+str(vehicle.location.local_frame.down)+ "global alt: "+str(vehicle.location.global_relative_frame.alt))
                
                    if alt>=6:
                       self.decrease_alt(5)
                    elif alt>=4:
                       self.decrease_alt(2.5)
                    else:
                    ...
                    marker_lat, goto_ned(x_cm,y_cm)
                
                        
                    marker_lon  = get_location_metres(uav_location, north*0.01, east*0.01)
                    ...
                    logging.info( "UAV Location    Lat = %.7f  Lon = %.7f"%(uav_location.lat, uav_location.lon))
                    logging.info( "Commanding to   Lat = %.7f  Lon = %.7f"%(location_marker.lat, location_marker.lon))
               ...
               if z_m < 10:
                   logging.info('Marker went outside camera frame increasing altitude')
                   location_marker = LocationGlobalRelative(
                       uav_location.lat, uav_location.lon, z_m+0.5)
                   vehicle.simple_goto(location_marker)
               else:
                   logging.info('cannot detect')
                   logging.info(" -->>COMMANDING TO LAND<<")
                   vehicle.mode = "LAND"
                   logging.info("Exiting.........")
