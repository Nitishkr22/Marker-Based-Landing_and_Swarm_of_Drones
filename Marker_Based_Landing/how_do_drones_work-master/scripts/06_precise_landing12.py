...
Be sure the RASPI CAME
RA driver is correctly acivated -> type the following
...
camera_matrix = np.loadtxt(
    calib_path+'cameraMatrix_webcam.txt', delimiter=',')
...
while True:
    ...
    #             uav_location.lat, uav_location.long, uav_location.alt-land_speed_cms)
    ...
    if marker_found:
        ...
        if time.time() >= time_0 + 1.0/freq_send:
            ...
            condition_yaw(0)
            if z_m <= 5.5 and z_m > 3.5:
                print("Applying correction 1, Descending...")
                goto_position_target_local_ned(-x_cm/100.0, -y_cm/100, -3.5)
                time.sleep(5)

            if z_m <= 3.5 and z_m > 2:
                print("Applying correction 2, Descending...")
                goto_position_target_local_ned(-x_cm/100.0, -y_cm/100, -2)
                time.sleep(5)

            if z_m <= 2:
                print("Applying correction 3, Descending...")
                goto_position_target_local_ned(-x_cm/100.0, -y_cm/100, 0.5)
                time.sleep(5)
