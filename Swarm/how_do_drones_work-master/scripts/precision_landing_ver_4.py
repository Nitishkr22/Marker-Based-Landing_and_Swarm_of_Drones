...
x = threading.Thread(target=thread_function,
                     args=(aruco_tracker,), daemon=True)
x.start()
# arm_and_takeoff(5)
yaw_flag = 0
target_altitude = 6
...
while True:
    ...
    if marker_found:
        ...
        if time.time() >= time_0 + 1.0/freq_send:
            ...
            print(" ")
            print("Altitude = %.0fm" % range_sensor_alt_m)
            print(
            ...
            goto_position_target_local_ned(
                vehicle.location.local_frame.north-y_cm/100.0, vehicle.location.local_frame.east+x_cm/100.0, -vehicle.location.global_relative_frame.alt)
            time.sleep(15)
            print('Descending...')
            decrease_alt(target_altitude)
            time.sleep(10)
            print('Decreased altitude to', target_altitude)
            target_altitude = target_altitude/2
        ...
        uav_location = vehicle.location.global_relative_frame
        z_m = uav_location.alt
        #range_sensor_alt_m = round(vehicle.rangefinder.distance,3)
