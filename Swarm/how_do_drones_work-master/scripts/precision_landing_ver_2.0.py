...
camera_matrix = np.loadtxt(
    calib_path+'cameraMatrix_fire.txt', delimiter=',')
camera_distortion = np.loadtxt(
    calib_path+'cameraDistortion_fire.txt', delimiter=',')
...
x = threading.Thread(target=thread_function,args=(aruco_tracker,), daemon=True)
x.start()
#arm_and_takeoff(5)
yaw_flag =0
...
while True:
    ...
    if marker_found:
        ...
        if time.time() >= time_0 + 1.0/freq_send:
            ...
            # print( ""
            print(" ")
            print("Altitude = %.0fm" % z_m)
            print(
                "Marker found x = %5.0f cm  y = %5.0f cm " % (x_cm, y_cm))
                
            if z_m > 6:
                print("Applying correction 1/3, Descending...")
                goto_position_target_local_ned(vehicle.location.local_frame.north-y_cm/100.0, vehicle.location.local_frame.east+x_cm/100.0, -6)
                time.sleep(25)

            elif z_m <= 6.5 and z_m > 3.5:
                print("Applying correction 2/3, Descending...")
                goto_position_target_local_ned(vehicle.location.local_frame.north-y_cm/100.0, vehicle.location.local_frame.east+x_cm/100.0, -3)
                time.sleep(25)

            else:
                print("Applying correction 3/3, Descending...")
                goto_position_target_local_ned(vehicle.location.local_frame.north-y_cm/100.0, vehicle.location.local_frame.east+x_cm/100.0, -1.5)
                time.sleep(25)
           
            print('moving drone to, N=',vehicle.location.local_frame.north-y_cm/100.0,'and E=',vehicle.location.local_frame.east+x_cm/100.0)
            #north, east  = uav_to_ne(x_cm, y_cm, vehicle.attitude.yaw)
            print( "Marker N = %5.0f cm   E = %5.0f cm"%(-y_cm, x_cm))
        ...
        if z_m<8:
            ...
            location_marker         = LocationGlobalRelative(uav_location.lat, uav_location.lon, z_m+0.5)
            vehicle.simple_goto(location_marker)
        else:
            print('cannot detect')
            print(" -->>COMMANDING TO LAND<<")
